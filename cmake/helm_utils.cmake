function(define_helm_target)
    ...
    add_custom_target(${TARGET_NAME}-upload
            DEPENDS ${TARGET_NAME}
            COMMAND if [ $$\(${CURL_EXECUTABLE} -s -o /dev/null -w \"%{http_code}\" --request POST --user gitlab-ci-token:$$CI_JOB_TOKEN --form \"chart=@${CHART_NAME}-${CHART_VERSION}.tgz\" \"$ENV{CI_API_V4_URL}/projects/$ENV{CI_PROJECT_ID}/packages/helm/api/stable/charts\"\) != 201 ]\; then false\; fi
            WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/${CHART_ROOT}
    ...
    add_custom_target(${TARGET_NAME}-deploy
            COMMAND ${HELM_EXECUTABLE} repo add --username $$CI_REGISTRY_USER --password $$CI_REGISTRY_PASSWORD --force-update  assfire-suite $ENV{CI_API_V4_URL}/projects/$ENV{CI_PROJECT_ID}/packages/helm/stable
