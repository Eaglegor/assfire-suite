...
RoutingMetricsCollector::RoutingMetricsCollector(std::shared_ptr<prometheus::Exposer> exposer):
	exposer(exposer),
...
std::shared_ptr<RoutingMetricsCollector::RAIIStopwatch> RoutingMetricsCollector::routesRequestStopwatch(int count, RoutingMetricsCollector::RequestMode mode) const
{
	if(!exposer) return std::make_shared<RoutingMetricsCollector::RAIIStopwatch>();
	
	return std::make_shared<RoutingMetricsCollector::RAIIStopwatch>(
		[this,mode](long ns) {
			switch (mode)
			{
...
void RoutingMetricsCollector::addRequestedRoutes(int count, RoutingMetricsCollector::RequestMode mode) const
{
	if (!exposer) return;
...
void RoutingMetricsCollector::addEuclideanCalculatedRoutes(int count, RequestMode mode) const
{
	if (!exposer) return;
...
void RoutingMetricsCollector::addRandomCalculatedRoutes(int count, RequestMode mode) const
{
	if (!exposer) return;
...
void RoutingMetricsCollector::addRoutesCacheHits(int count) const
{
	if (!exposer) return;
...
void RoutingMetricsCollector::addRoutesCacheMisses(int count) const
{
	if (!exposer) return;
...
std::shared_ptr<RoutingMetricsCollector::RAIIStopwatch> assfire::RoutingMetricsCollector::routesCacheStopwatch(int count, RequestMode mode) const
{
	if(!exposer) return std::make_shared<RoutingMetricsCollector::RAIIStopwatch>();
